...
class MyvideoLink:
	...
	def getLink(self, cb_play, cb_err, title, url, token):
		...
		if id:
			...
			cb_err('No ID found')
		"""
		getPage(url, headers={'Content-Type':'application/x-www-form-urlencoded'}).addCallback(self.getStream, title, token).addErrback(cb_err)
	...
	def getStream(self, data, title, token):
